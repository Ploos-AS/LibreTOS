...
def main():
 ...
 hd=OUT/'hd'; hd.mkdir(parents=True,exist_ok=True); (hd/'AUTO').mkdir(exist_ok=True); probe=hd/'M6FAL.TOS'; result=hd/'M6FAL.TXT'; result.unlink(missing_ok=True); log=OUT/'hatari.log'
 ...
 effective={'profile_id':profile['id'],'machine':'falcon','cpu_level':3,'cpu_clock_mhz':16,'st_ram_mib':4,'addressing_bits':32,'mmu':True,'run_vbls':run_vbls,'guest_program':'C:\\M6FAL.TOS','result_file':'C:\\M6FAL.TXT','launch':'hatari-auto-with-gemdos-auto-dir'}
