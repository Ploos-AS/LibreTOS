...
def main():
 ...
 if profile.get('id')!='falcon030-68030-4m-512k-us': return fail('unexpected profile id')
 hd=OUT/'hd'; auto=hd/'AUTO'; auto.mkdir(parents=True,exist_ok=True); probe=auto/'M6FAL.PRG'; result=hd/'M6FAL.TXT'; result.unlink(missing_ok=True); log=OUT/'hatari.log'
 ...
 # The probe lives in C:\AUTO, so C: must actually be the boot drive.  Merely
 # attaching Hatari's GEMDOS drive exposes C: after boot but does not make TOS
 # execute its AUTO folder.  Force hard-disk boot and keep a generous Falcon
 # startup window.
 run_vbls=max(int(q['minimum_vbls']),5000)
 guest_program='C:\\AUTO\\M6FAL.PRG'
 effective={'profile_id':profile['id'],'machine':'falcon','cpu_level':3,'cpu_clock_mhz':16,'st_ram_mib':4,'addressing_bits':32,'mmu':True,'run_vbls':run_vbls,'guest_program':guest_program,'result_file':'C:\\M6FAL.TXT','launch':'gemdos-auto-boot','boot_from_hd':True,'startup_margin':'5000-vbl-minimum'}
 ...
 cmd=['hatari','--tos',str(ROM),'--machine','falcon','--memsize','4','--cpulevel','3','--cpuclock','16','--addr24','no','--mmu','on','--compatible',yn(bool(q['compatible_mode'])),'--fast-boot',yn(bool(q['fast_boot'])),'--sound','off','--confirm-quit','no','--benchmark','--run-vbls',str(run_vbls),'--harddrive',str(hd),'--boot-from-hd','yes','--protect-hd','off','--gemdos-case','upper','--log-file',str(log)]
