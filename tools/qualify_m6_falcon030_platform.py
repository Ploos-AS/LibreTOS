...
def main():
 ...
 # Build the diagnostic probe for the baseline 68000 ISA.  The previous
 # -m68020-60 binary entered the MiNT libc CPU startup guard and terminated
 # with Pterm(-1) before main() on this EmuTOS/Falcon profile.  The probe
 # itself validates the Falcon _CPU cookie, so no CPU qualification is lost.
 cp=subprocess.run(['m68k-atari-mint-gcc','-m68000','-O2','-s',f'-DPROFILE_NAME="{profile["id"]}"','-DRESULT_FILE="C:\\\\M6FAL.TXT"','-o',str(probe),str(SOURCE)],cwd=ROOT)
 if cp.returncode:return fail(f'guest probe compile exit {cp.returncode}')
 run_vbls=max(int(q['minimum_vbls']),5000)
 effective={'profile_id':profile['id'],'machine':'falcon','cpu_level':3,'cpu_clock_mhz':16,'st_ram_mib':4,'addressing_bits':32,'mmu':True,'run_vbls':run_vbls,'guest_program':'C:\\AUTO\\M6FAL.PRG','guest_probe_isa':'68000','result_file':'C:\\M6FAL.TXT','launch':'gemdos-auto-with-gemdos-trace','trace':'gemdos','startup_margin':'5000-vbl-minimum'}
