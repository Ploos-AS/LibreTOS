...
def main():
 ...
 # Falcon desktop/AES startup is not a prerequisite for this BIOS/XBIOS
 # platform probe. Run it through TOS' boot-time AUTO mechanism instead of
 # Hatari --auto, which waits for GEM desktop startup and never fired on the
 # current Falcon ROM. Keep a generous VBL window for the boot-time probe.
 run_vbls=max(int(q['minimum_vbls']),5000)
 guest_program='C:\\AUTO\\M6FAL.PRG'
 effective={'profile_id':profile['id'],'machine':'falcon','cpu_level':3,'cpu_clock_mhz':16,'st_ram_mib':4,'addressing_bits':32,'mmu':True,'run_vbls':run_vbls,'guest_program':guest_program,'result_file':'C:\\M6FAL.TXT','launch':'gemdos-auto-boot','startup_margin':'5000-vbl-minimum'}
