...
SOURCE = ROOT / "tests" / "m2" / "gemdos_probe.c"
OUT = ROOT / "build" / "m2" / "gemdos"
HD = OUT / "hd"
PROBE = HD / "GEMDOS.TOS"
RESULT = HD / "M2GEMDOS.TXT"
LOG = OUT / "hatari.log"
...
def fail(message: str) -> None:
    print(f"M2.3 GEMDOS regression: FAIL ({message})", file=sys.stderr)
    if LOG.exists():
...
def main() -> None:
    ...
    if not SOURCE.is_file():
        fail("guest probe source missing")
    ...
    run_vbls = max(int(q["minimum_vbls"]), 1000)
    ...
    (OUT / "PROBE.sha256").write_text(f"{sha256(PROBE)}  GEMDOS.TOS\n", encoding="utf-8")
    ...
    effective = {
        ...
        "st_ram_mib": ram_mib,
        "run_vbls": run_vbls,
        "guest_program": r"C:\GEMDOS.TOS",
        "result_file": r"C:\M2GEMDOS.TXT",
    }
    ...
    print(f"M2.3 GEMDOS regression: PASS ({profile['id']})")
