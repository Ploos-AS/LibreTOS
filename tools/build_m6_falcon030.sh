...
SRC_DIR=${SRC_DIR:-"$ROOT/build/emutos-m6-falcon030"}
OUT_DIR=${OUT_DIR:-"$ROOT/build/m6/falcon030"}
ARTIFACT="$OUT_DIR/LibreTOS-Falcon030-68030-512k-us.img"
...
make -C "$SRC_DIR" UNIQUE=us 512
ROM="$SRC_DIR/etos512us.img"
test -f "$ROM"
cp "$ROM" "$ARTIFACT"

test "$(stat -c %s "$ARTIFACT")" -eq $((512 * 1024))
...

{
    ...
    echo "profile=falcon030-68030-4m-512k-us"
    echo "artifact=$(basename "$ARTIFACT")"
    echo "rom_kib=512"
} > "$OUT_DIR/BUILDINFO.txt"

stat -c 'Falcon030 ROM size: %s bytes' "$ARTIFACT"
