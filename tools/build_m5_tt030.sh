...
SRC_DIR=${SRC_DIR:-"$ROOT/build/emutos-m5-tt030"}
OUT_DIR=${OUT_DIR:-"$ROOT/build/m5/tt030"}
ARTIFACT="$OUT_DIR/LibreTOS-TT030-68030-512k-us.img"
...

{
    ...
    echo "profile=tt030-68030-4m-16mtt-512k-us"
    echo "artifact=$(basename "$ARTIFACT")"
    echo "rom_kib=512"
} > "$OUT_DIR/BUILDINFO.txt"

stat -c 'TT030 ROM size: %s bytes' "$ARTIFACT"
