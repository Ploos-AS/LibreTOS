...
SOURCE = ROOT / "tests" / "m3" / "ste_enhanced_probe.c"
OUT = ROOT / "build" / "m3" / "ste-enhanced"
HD = OUT / "hd"
PROBE = HD / "STEENH.TOS"
RESULT = HD / "M3ENH.TXT"
LOG = OUT / "hatari.log"
...
def fail(message: str) -> None:
    ...
    (OUT / "RESULT.txt").write_text(
        f"M3.4 STe enhanced hardware regression: FAIL\nreason={message}\n",
        encoding="utf-8",
    )
    print(f"M3.4 STe enhanced hardware regression: FAIL ({message})", file=sys.stderr)
    raise SystemExit(1)
...
def main() -> None:
    ...
    effective = {
        ...
        "guest_program": r"C:\STEENH.TOS",
        "result_file": r"C:\M3ENH.TXT",
        "checks": ["hardware-scroll-rw", "dma-sound-mode-rw", "blitter-toggle"],
    }
    ...
    if fields.get("stage") != "complete":
        fail("guest result incomplete")
    if fields.get("hscroll_written") != fields.get("hscroll_read"):
        fail("hardware scroll readback mismatch")
    if (int(fields.get("dma_mode_read", "0"), 0) & 0x03) != 0x03:
        fail("DMA sound mode readback mismatch")
    if (int(fields.get("blit_disabled", "0"), 0) & 0x01) != 0:
        fail("blitter did not disable")
    if (int(fields.get("blit_enabled", "0"), 0) & 0x03) != 0x03:
        fail("blitter did not enable")

    normalized = "\n".join(f"{k}={fields[k]}" for k in sorted(fields)) + "\n"
    (OUT / "RESULT.txt").write_text(normalized, encoding="utf-8")
    print(f"M3.4 STe enhanced hardware regression: PASS ({profile['id']})")
