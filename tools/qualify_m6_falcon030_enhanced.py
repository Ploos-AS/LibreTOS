...
def main():
 ...
 hd=OUT/'hd'; hd.mkdir(parents=True,exist_ok=True); probe=OUT/'M6FALE.PRG'; floppy=OUT/'m6fale-auto.st'; result=hd/'M6FALE.TXT'; result.unlink(missing_ok=True); log=OUT/'hatari.log'
 ...
 guest_program='A:\\AUTO\\M6FALE.PRG'
 effective={'profile_id':profile['id'],'machine':'falcon','cpu_level':3,'cpu_clock_mhz':16,'st_ram_mib':4,'addressing_bits':32,'mmu':True,'sound_hz':44100,'run_vbls':run_vbls,'guest_program':guest_program,'result_file':'C:\\M6FALE.TXT','launch':'floppy-auto-boot','boot_floppy':floppy.name,'startup_margin':'5000-vbl-minimum','qualified':['VIDEL identification','Falcon sound capability discovery'],'observed':['FPU cookie'],'excluded':['DSP execution semantics','IDE read/write semantics','NVRAM persistence','external audio fidelity']}
 OUT.mkdir(parents=True,exist_ok=True); (OUT/'PROFILE.json').write_text(json.dumps(profile,indent=2,sort_keys=True)+'\n'); (OUT/'ROM.sha256').write_text(f'{sha256(ROM)}  {ROM.name}\n'); (OUT/'PROBE.sha256').write_text(f'{sha256(probe)}  {probe.name}\n'); (OUT/'FLOPPY.sha256').write_text(f'{sha256(floppy)}  {floppy.name}\n'); (OUT/'HATARI_PROFILE.json').write_text(json.dumps(effective,indent=2,sort_keys=True)+'\n')
 cmd=['hatari','--tos',str(ROM),'--machine','falcon','--memsize','4','--cpulevel','3','--cpuclock','16','--addr24','no','--mmu','on','--compatible',yn(bool(q['compatible_mode'])),'--fast-boot',yn(bool(q['fast_boot'])),'--sound','44100','--confirm-quit','no','--benchmark','--run-vbls',str(run_vbls),'--disk-a',str(floppy),'--harddrive',str(hd),'--protect-hd','off','--gemdos-case','upper','--log-file',str(log)]
