...
def main():
 ...
 effective={'profile_id':profile['id'],'machine':'falcon','cpu_level':3,'cpu_clock_mhz':16,'st_ram_mib':4,'addressing_bits':32,'mmu':True,'sound_hz':44100,'run_vbls':run_vbls,'guest_program':'A:\\AUTO\\M6FALE.PRG','result_file':'A:\\M6FALE.TXT','launch':'floppy-plus-explicit-hatari-auto','startup_margin':'5000-vbl-minimum','qualified':['VIDEL identification','Falcon sound capability discovery'],'observed':['FPU cookie'],'excluded':['DSP execution semantics','IDE read/write semantics','NVRAM persistence','external audio fidelity']}
 ...
 cmd=['hatari','--tos',str(ROM),'--machine','falcon','--memsize','4','--cpulevel','3','--cpuclock','16','--addr24','no','--mmu','on','--compatible',yn(bool(q['compatible_mode'])),'--fast-boot',yn(bool(q['fast_boot'])),'--sound','44100','--confirm-quit','no','--benchmark','--run-vbls',str(run_vbls),'--disk-a',str(floppy),'--auto','A:\\AUTO\\M6FALE.PRG','--log-file',str(log)]
