...
def main():
 ...
 # M6.4 needs BIOS/XBIOS/cookie state, not a running GEM desktop. Execute
 # from the boot-time AUTO folder so Falcon desktop startup cannot block the
 # qualification probe. Keep the same generous runtime window as M6.3.
 run_vbls=max(int(q['minimum_vbls']),5000)
 guest_program='C:\\AUTO\\M6FALE.PRG'
 effective={'profile_id':profile['id'],'machine':'falcon','cpu_level':3,'cpu_clock_mhz':16,'st_ram_mib':4,'addressing_bits':32,'mmu':True,'sound_hz':44100,'run_vbls':run_vbls,'guest_program':guest_program,'result_file':'C:\\M6FALE.TXT','launch':'gemdos-auto-boot','startup_margin':'5000-vbl-minimum','qualified':['VIDEL identification','Falcon sound capability discovery'],'observed':['FPU cookie'],'excluded':['DSP execution semantics','IDE read/write semantics','NVRAM persistence','external audio fidelity']}
