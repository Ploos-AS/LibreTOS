...
def fail(msg: str) -> None:
    raise SystemExit(f"m6-profile: FAIL: {msg}")


def main() -> None:
    if not PROFILE.is_file():
        fail("canonical Falcon030 profile missing")

    p = json.loads(PROFILE.read_text(encoding="utf-8"))
    if p.get("schema") != 1:
        fail("schema must be 1")
    if p.get("id") != "falcon030-68030-4m-512k-us":
        fail("unexpected profile id")

    m = p.get("machine", {})
    expected_machine = {
        "family": "Atari Falcon030",
    ...
    for key in (
        "falcon_video", "blitter", "dma_sound", "dsp56001", "ide",
        "nvram", "real_time_clock", "mmu", "fpu_optional",
    ):
    ...
    if q.get("must_regress") != [
        ...
        "tt030-68030-4m-16mtt-512k-us",
    ]:
        fail("regression gate mismatch")

    registry = json.loads(TARGETS.read_text(encoding="utf-8"))
    t = next((x for x in registry.get("targets", []) if x.get("id") == "falcon030-68030"), None)
    if t is None:
        fail("Falcon030 target missing from registry")
    if t.get("profile") != "config/m6-falcon030-68030.json":
        fail("registry profile mismatch")
    if t.get("cpu") != "68030" or t.get("rom_kib") != 512:
        fail("registry CPU/ROM contract mismatch")
    if t.get("artifact") != "LibreTOS-Falcon030-68030-512k-us.img":
        fail("registry artifact mismatch")

    print("m6-profile: PASS")
